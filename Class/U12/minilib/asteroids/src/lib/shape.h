...
class shape : public ml5::object, MI5_DERIVE(shape, ml5::object) {
	...
	public:
		...
		bool is_colliding(const wxPoint &point) const {
			return is_colliding(point, 0);
		}

		//returns true if point is inside bounding box + an additional zone radius around the shape
		bool is_colliding(const wxPoint& point, const int radius) const {
			wxPoint top_left = m_box.GetTopLeft();
			wxPoint bot_right = m_box.GetBottomRight();

			int x1 = std::max(top_left.x, bot_right.x) + radius;
			int x2 = (x1 == top_left.x ? bot_right.x : top_left.x) - radius;
			int y1 = std::max(top_left.y, bot_right.y) + radius;
			int y2 = (y1 == top_left.y ? bot_right.y : top_left.y) - radius;
		...
		virtual void draw_debug(context_t& context, wxColor col) {}
